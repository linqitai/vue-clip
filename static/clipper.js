...
export default {
  ...
  install (Vue) {
    ...
    Vue.prototype.createElement = function () {
      ...
      let str = '<div><img id="clip_image" src="originUrl"></div><button type="button" id="cancel_clip">取消</button><button type="button" id="clip_button" :disable="btnDisable">确定</button>'
      str += '<div class="crop_loading" id="crop_loading"><div class="crop_content"><img src="../static/loading.gif"><div class="crop_text">图片上传中</div></div></div>'
    ...
    Vue.prototype.initFunction = function () {
      ...
      this.addEvent(this.clickBtn, 'click', function () {
        document.getElementById('clip_button').style.background = 'gray'
        console.log('background：' + document.getElementById('clip_button').style.background)
        // 显示loading
      ...
      this.addEvent(this.cancelBtn, 'click', function () {
        self.destoried()
        console.log('src:' + self.resultObj.src)
        if (self.resultObj.src.indexOf('data') === -1) {
          self.hasImg = false
        }
      })
      // 清空input的值
    ...
    Vue.prototype.crop = function (compressRaTio) {
      ...
      if (imgData.length < (300 * 1024)) {
        console.log(`图片大小${imgData.length},图片小于300k，直接上传`)
